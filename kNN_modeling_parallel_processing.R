...
foreach(n = sample_sizes, .packages = c("tidymodels", "kknn", "dplyr")) %dopar% {
  ...
  # Save predictions
  filename <- paste0("~/PROJECT/model_preds/kNN/knn_test_predictions_", format(n, scientific = FALSE), ".rds")
  ...
  log_filename <- paste0("~/PROJECT/runtime_analysis/knn_model_log_", format(n, scientific = FALSE), ".txt")
